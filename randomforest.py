...
BASELINE_MAX_DEPTH = None
BASELINE_MAX_FEATURES = "sqrt"
BASELINE_MIN_SAMPLES_LEAF = 1
BASELINE_RF_SEED = 44444
...
# ============================================================
# DATA SPLIT BY CAR ID (stratified)
# ============================================================
...
# ============================================================
# RANDOM FOREST PIPELINE WITH FIXED SEVERITY TRAINING
# ============================================================
...
def run_rf_pipeline(
    ...
    print(f"[RF] PCA={pca_dim}, n_estimators={n_estimators}, "
    ...
    rf_cat.fit(X_train_pca, y_train_cats)

    # ---- 4. Train RF for severity of EACH part (only on damaged samples) ----
    ...
    for p in range(NUM_PARTS):
        ...
        # Check if we have multiple severity classes
        unique_sevs = np.unique(y_train_sev_part)
        if len(unique_sevs) < 2:
            print(f"    {LOCATION_NAMES[p]:<15} Skipped (only severity class {unique_sevs[0]})")
            rf_sevs.append(None)
            continue

        # Train classifier for this specific part
        rf_sev_part = RandomForestClassifier(
            n_estimators=n_estimators,
            max_depth=max_depth,
            max_features=max_features,
            min_samples_leaf=min_samples_leaf,
        ...
        rf_sev_part.fit(X_train_part, y_train_sev_part)
        rf_sevs.append(rf_sev_part)

        print(f"    {LOCATION_NAMES[p]:<15} Trained on {n_damaged} samples, classes: {sorted(unique_sevs)}")

    # ====================================================
    # EVALUATION - CATEGORIES
    # ====================================================

    proba_list = rf_cat.predict_proba(X_val_pca)
    y_pred_cats = np.zeros_like(y_val_cats)
    for p in range(NUM_PARTS):
        y_pred_cats[:, p] = (proba_list[p][:, 1] >= threshold).astype(int)
    ...
    all_scores = np.concatenate([proba_list[p][:, 1] for p in range(NUM_PARTS)])
    ...
    # Predict severity for each part separately
    for p in range(NUM_PARTS):
        if rf_sevs[p] is not None:
            # Predict for ALL validation samples (classifier will handle it)
            y_pred_sevs[:, p] = rf_sevs[p].predict(X_val_pca)
    ...
    for i, name in enumerate(LOCATION_NAMES):
        if np.isnan(per_part_sev_acc[i]):
            status = "N/A (no damaged parts)" if rf_sevs[i] is not None else "N/A (no trained model)"
    ...
    if run_dir is not None:
        ...
        for i, name in enumerate(LOCATION_NAMES):
            ...
            predictions_df[f'Pred_Prob_{name}'] = proba_list[i][:, 1]
        ...
        if make_plots:
            ...
            plt.plot(fpr_all, tpr_all, label=f"RF Category (AUC = {auc_all:.4f})", linewidth=2)
            plt.plot([0, 1], [0, 1], "k--", label="Random", linewidth=1)
            plt.xlabel("False Positive Rate", fontsize=12)
            plt.ylabel("True Positive Rate", fontsize=12)
            plt.title("Random Forest – Global Category ROC", fontsize=14)
...
baseline_summary = run_rf_pipeline(
...
print("\n" + "=" * 80)
print("RANDOM FOREST HYPERPARAMETER SWEEP")
print("=" * 80)
...
RF_SEEDS = [44444, 33]
THRESHOLDS = [0.35, 0.42, 0.50]


sweep_records = []

total_runs =  (len(PCA_LIST) * len(N_ESTIMATORS_LIST) *
              len(MAX_DEPTH_LIST) * len(MAX_FEATURES_LIST) *
              len(MIN_SAMPLES_LEAF_LIST) * len(RF_SEEDS) * len(THRESHOLDS))
...
if sweep_records:
    ...
    print("\n" + "="*80)
    print(f"✓ Saved RF sweep global summary to: {summary_path}")
    print("="*80)
